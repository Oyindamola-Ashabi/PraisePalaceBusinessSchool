    ...
    <main className="about-page-content">
      ...
      <section className="about-hero-section">
        ...
        <div className="hero-content">
          <h1 className="water-text" data-text="Raising Principled Leaders for Tomorrow">
            Raising Principled Leaders for Tomorrow
          </h1>
          <p>
      ...
      <section className="academy-story-section" ref={storyRef}>
        <div className="academy-container">
          ...
          <div className={`academy-text-column ${isStoryVisible ? 'fade-in-left' : 'is-hidden'}`}>
            <h2 className={`academy-title pop-title ${isStoryVisible ? 'visible' : ''}`}>
              {'The ‘Why’ Behind PraisePalace Business School'.split('').map((char, index) => (
        ...
        <div className={`container ${isMissionVisible ? 'fade-up' : 'is-hidden'}`}>
          <div className="mission-vision-grid">
            <div className="info-box">
              <h3 className={`pink-label pop-title ${isMissionVisible ? 'visible' : ''}`}>
      ...
      <section className="offer-section" ref={offerRef}>
        <div className={`container ${isOfferVisible ? 'fade-up' : 'is-hidden'}`}>
          <h2 className={`academy-title pop-title ${isOfferVisible ? 'visible' : ''}`}>
            {'What else does our School offer?'.split('').map((char, index) => (
              <span key={index} style={{ animationDelay: `${index * 0.06}s` }}>
                {char === ' ' ? '\u00A0' : char}
              </span>
            ))}
